...
参数个数或者类型不同，在函数调用时，能够准确识别到对应的函数定义，既然
函数名相同，那么它们之间肯定有些非常密切的关联，否则另起一个函数不是更
明确？比如想将两个float型的数相加，就可以定义`float add(float, float)`
...
> 除了函数对象，还有数据对象，广义上的，一个简单类型变量，如果`int a`,变量a
